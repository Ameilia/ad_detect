...
void getShapeHistogram(vector<Point>& shape, vector<float>& histogram, int dim = 100) {
	...
	//for (int i = 2; i < dim * 2 - 3; i++) {
	//  if (i >= dim - 2 && i <= dim + 2)continue;
	//  histogram[i] = temp[i - 2] * 0.05 + temp[i - 1] * 0.2 + temp[i] * 0.5 + temp[i + 1] * 0.2 + temp[i + 2] * 0.05;
	//}
...
int main(int argc, char* argv[]) {
	...
	string rgb_filename = "C:\\Users\\Am\\Downloads\\dataset2\\dataset2\\Videos\\data_test2.rgb";
	int width = 480;
	int height = 270;
	...
	Mat subway_image = imread("C:\\Users\\Am\\Downloads\\dataset2\\dataset2\\Brand Images\\Mcdonalds_logo.jpg");
	...
	Mat starbucks_image = imread("C:\\Users\\Am\\Downloads\\dataset2\\dataset2\\Brand Images\\nfl_logo.bmp");
	...
	Mat motionHist = Mat::zeros(200, 480, CV_8UC3);
	...
	while (!feof(IN_FILE)) {
		...
		if (prevgray.data) {
			...
			if (frame_count % 480 == 0) motionHist = Mat::zeros(motionHist.size(), motionHist.type());
			motionHist(Range(max(200 - meanMagnitude, 0.0), 200), Range(frame_count % 480, frame_count % 480 + 1)) = Scalar(255, 255, 255);
			motionHist.at<cv::Vec3b>(Point(frame_count % 480, max(200 - averageMag, 0.0))) = Vec3b(0, 0, 255);
